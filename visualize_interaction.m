function visualize_interaction(size_range, fixed_size, threshold, show_histogram)
    ...
    % Определение границ для нормализации с исключением экстремальных значений
    critical_min = -threshold;
    critical_max = threshold;
    ...
    for i = 1:length(positive_indices)
        if interaction_array(positive_indices(i), 1) > threshold
            ...
            normalized_positive = (interaction_array(positive_indices(i), 1) - critical_min) / (critical_max - critical_min);
            color_index = round(normalized_positive * (size(colormap_1, 1) - 1)) + 1;
    ...
    for i = 1:length(negative_indices)
        if abs(interaction_array(negative_indices(i), 1)) > threshold
            ...
            normalized_negative = (interaction_array(negative_indices(i), 1) - critical_min) / (critical_max - critical_min);
            color_index = round(normalized_negative * (size(colormap_2, 1) - 1)) + 1;
    ...
    colorbar;
    caxis([critical_min, critical_max]);
    hold off;
    ...
    addlistener(h_slider, 'Value', 'PostSet', @(src, event) updateSlice(slice_ax, interaction_array, event.AffectedObject.Value, critical_min, critical_max));
    ...
    updateSlice(slice_ax, interaction_array, min(interaction_array(:, 4)), critical_min, critical_max);
...
function updateSlice(slice_ax, interaction_array, slice_value, critical_min, critical_max)
    ...
    if ~isempty(slice_data)
        ...
        C_norm = (C - critical_min) / (critical_max - critical_min);
        surf(slice_ax, X, Y, Z, C_norm);
        caxis([critical_min, critical_max]); % Установить те же пределы цветовой шкалы, что и для объёмной визуализации
        xlabel('X');
        ylabel('Y');
        zlabel('Z');
